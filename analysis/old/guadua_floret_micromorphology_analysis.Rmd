...
guaduafloret <- guaduafloret %>%
  select(-c(lemma_stomata, palea_sulcus_macro, palea_stomata_freq, palea_subsidiary_triangular, palea_subsidiary_dome, palea_subsidiary_parallel)) # remove columns with NA or no variation
...
#### Model comparison excluding region
...
guadua_Habit_Habitat <- lm.rrpp(guadua_floret_y_dist_matrix ~ floretdat3$Habit + floretdat3$Habitat,
                         print.progress = FALSE)
guadua_full2 <- lm.rrpp(guadua_floret_y_dist_matrix ~ floretdat3$Habit + floretdat3$Habitat, 
                print.progress = FALSE)
