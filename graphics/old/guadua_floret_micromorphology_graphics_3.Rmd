...
For this PCoA, individual specimens are colored by the habit typical of their species. Three habitat groups were used: big and erect, leaning/climbing, and small arching. This PCoA is used as Figure 6 in the manuscript.
```{r}
...
For this PCoA, individual specimens are colored by the habitat typical of their species. Three habitat groups were used: Forest, river, and savanna. This PCoA is used as Figure 5 in the manuscript.
```{r}
...
For this PCoA, individual specimens are colored by species. This PCoA is used as Appendix Figure 6 in the manuscript.
```{r}
group.colors <- c("g_aculeata" = "#7b3294", "g_amplexifolia" = "#1b7837",
                  "g_angustifolia" = "#f1a340", "g_calderoniana" = "#9970ab","g_chacoensis" = "#5aae61",
                  "g_ciliata" = "#40004b", "g_glomerata" = "#00441b", 
...
speciespcoa <- nice_df %>% 
 ...
 scale_color_manual(name = "Species", 
                     breaks = c("g_aculeata", "g_amplexifolia","g_angustifolia",
                                ...
                                "g_weberbaueri", "g_aff_sarcocarpa",
                                "g_aff_weberbaueri"),
...
From this we can obtain eigenvalues and then plot them to show the percent variation explained along each PCoA axis. As seen in the plot below, PCoA axes 1 and 2 are responsible for less than half of variation in the PCoA. Elbow is at approximately the eighth principal coordinate axis. Variation becomes negligible at approximately the 30th principal coordinate axis. This image was used as Figure A.4 - Appendix Figure 4 in the manuscript.
```{r}
...
guaduafloret_general_forest <- df_guaduafloret %>%
  filter(general_habitat == "general_forest")
```

##General forest: Pie chart of simple papillae on abaxial lemma surface presence/absence
```{r}
#Group by simple papillae on abaxial lemma surface only and obtain percentage present/absent
guaduafloret_general_forest_lemma_papillae_simple_per <- guaduafloret_general_forest %>% 
...
#view(guaduafloret_general_forest_lemma_papillae_simple_per)

#Create labeled pie chart of percentage of general forest specimens with simple papillae on lemma abaxial surface
ggplot(guaduafloret_general_forest_lemma_papillae_simple_per, aes(x = "", y = perc, fill = lemma_papillae_simple)) +
...
forest_pie_lemma_simple_papillae <- ggplot(guaduafloret_general_forest_lemma_papillae_simple_per, aes(x = "", y = perc, fill = lemma_papillae_simple)) +
  geom_col() +
  coord_polar(theta = "y") +
...
##General forest: Pie chart of simple papillae on abaxial palea surface presence/absence
```{r}
#Group by simple papillae on abaxial palea surface only and obtain percentage present/absent
guaduafloret_general_forest_palea_papillae_simple_per <- guaduafloret_general_forest %>% 
...
#view(guaduafloret_general_forest_palea_papillae_simple_per)

#Create labeled pie chart of percentage of general forest specimens with simple papillae on palea abaxial surface
ggplot(guaduafloret_general_forest_palea_papillae_simple_per, aes(x = "", y = perc, fill = palea_papillae_simple)) +
  geom_col(color = "black") +
  geom_label(aes(label = labels),
             position = position_stack(vjust = 0.5),
             show.legend = FALSE) +
...
forest_pie_palea_simple_papillae <- ggplot(guaduafloret_general_forest_palea_papillae_simple_per, aes(x = "", y = perc, fill = palea_papillae_simple)) +
  geom_col() +
  coord_polar(theta = "y") +
...
##General forest: Pie chart of bilobate horizontally elongated silica bodies on abaxial palea surface presence/absence
```{r}
...
guaduafloret_general_forest_palea_bilobate_horizontal_per <- guaduafloret_general_forest %>% 
...
#view(guaduafloret_general_forest_palea_bilobate_horizontal_per)

#Create labeled pie chart of percentage of general forest specimens with simple papillae on palea abaxial surface
ggplot(guaduafloret_general_forest_palea_bilobate_horizontal_per, aes(x = "", y = perc, fill = palea_bilobate_horizontal)) +
  geom_col(color = "black") +
  geom_label(aes(label = labels),
             position = position_stack(vjust = 0.5),
             show.legend = FALSE) +
...
forest_pie_palea_bilobate_horizontal <- ggplot(guaduafloret_general_forest_palea_bilobate_horizontal_per, aes(x = "", y = perc, fill = palea_bilobate_horizontal)) +
  geom_col() +
  coord_polar(theta = "y") +
  guides(fill = guide_legend(title = "Bilobate horizontally elongated silica bodies
